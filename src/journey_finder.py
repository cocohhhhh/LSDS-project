...
class JourneyFinder:
    def __init__(self, timetable, footpaths, stops_info, delay_predictor, stops_matching):
        """
    ...
    def find_journeys(self, start_station_id, end_station_id, arrival_datetime: str, day_of_week: str, confidence_threshold=0.7):
        """
        ...
        print('journeys found, checking confidence levels...')
        
        journey_confidences = []
        for journey in journeys:
            stations_ids, timestamps = [], []
            for i in range(len(journey[:-1])):
                if journey[i]['transport'] != 'walking' and journey[i]['transport'] != journey[i+1]['transport']:
                    stations_ids.append(self.stops_matching[journey[i]['arrival_stop']])
                    timestamps.append(journey[i]['arrival_time'])

            delay_predictions_dict = self.delay_predictor.predict(stations_ids, timestamps)

            print(delay_predictions_dict)
            
            confidence = journey_confidence_on_arrival_delay_predictions(
                journey,
                delay_predictions_dict,
                arrival_datetime,
                self.stops_matching
            )
            journey_confidences.append(confidence)

            print(journey_confidences)
            
        print('confidence levels found...')

        if all(x < confidence_threshold for x in journey_confidences):
            print("no journey found with high enough confidence!")
    ...
    def __get_latest_arrival_journey(self, source_stop_id: str, destination_stop_id: str, arrival_time: str, day_of_week: str) -> list:
        ...
        filtered_timetable = self.timetable[self.timetable[day_of_week] == True]

        S, _ = self.__connection_scan_latest_arrival(filtered_timetable, self.footpaths, source_stop_id, destination_stop_id, arrival_time)
        ideal_journey = self.__journey_extraction_latest_arrival(S, source_stop_id, destination_stop_id, arrival_time)
        return ideal_journey
    

    def __find_list_of_journeys(self, source_stop_id: str, destination_stop_id: str, arrival_time: str, day_of_week: str, num_journeys: int = 5) -> list:
    ...
    def find_and_plot_journeys(self, start_station_name_id: str, end_station_name_id: str, arrival_datetime: str, day: str, confidence_threshold=0.7):
        ...
        Args:
            ...
            day (str): The day of week of the journey.
        """
        ...
        if start_station_id == end_station_id:
            print("Destination should be different than source")
            return
